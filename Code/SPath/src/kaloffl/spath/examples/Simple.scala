...
object Simple {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val matLight = new LightMaterial(Color.WHITE, 8f, 8f)
    ...
    val coloredSpheres = SceneNode(Array(
      SceneNode(
        ...
        matMirror),
      SceneNode(
        new Sphere(Vec3d(-1.0f, 1.5f, 4.0f), 1.5f),
        matGlass),

      SceneNode(
        new Sphere(Vec3d(-3.5f, 6.0f, 3.0f), 1),
        matLight),
      ...
      SceneNode(
        ...
        matWhiteDiffuse),
      SceneNode(
        AABB(Vec3d(0, 16.5, 0), Vec3d(16, 1, 24)),
        matWhiteDiffuse),
      SceneNode(
        AABB(Vec3d(8.5f, 8, 0), Vec3d(1, 16, 24)),
        matBlueDiffuse),
      SceneNode(
        AABB(Vec3d(-8.5f, 8, 0), Vec3d(1, 16, 24)),
        matRedDiffuse),
      SceneNode(
        AABB(Vec3d(0, 8, -12.5f), Vec3d(16, 16, 1)),
        matGreenDiffuse),
      SceneNode(
        AABB(Vec3d(0, 8, 12.5), Vec3d(16, 16, 1)),
        matWhiteDiffuse)))

    val focus = Vec3d(-4, 2.5, 3)
    val position = Vec3d(2, 2.5, 11)
    val forward = (focus - position).normalize
    val lowCamera = new Camera(position, forward, Vec3d.UP, 0.0f, 13);
