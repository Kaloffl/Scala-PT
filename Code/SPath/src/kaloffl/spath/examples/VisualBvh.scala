...
object VisualBvh {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val boxRed = SceneNode(bunny.enclosingAABB, matGlassRed)
    val boxGreen = bunny.root.children.map { n ⇒ SceneNode(n.hull, matGlassGreen) }
    val boxBlue = bunny.root.children.flatMap { _.children }.map { n ⇒ SceneNode(n.hull, matGlassBlue) }

    val boxes = SceneNode((0 until 5).foldLeft(bunny.root.children) { (ns, i) ⇒
